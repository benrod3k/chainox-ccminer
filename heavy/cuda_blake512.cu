...
// Folgende Definitionen spã³¥r durch header ersetzen
typedef unsigned char uint8_t;
typedef unsigned int uint32_t;
//typedef unsigned long long uint64_t;

// globaler Speicher fr alle HeftyHashes aller Threads
extern uint32_t *d_heftyHashes[8];
extern uint32_t *d_nonceVector[8];

// globaler Speicher fr unsere Ergebnisse
...
// Diese Makros besser nur fr Compile Time Konstanten verwenden. Sie sind langsam.
#define SWAP32(x) \
...
template <int BLOCKSIZE> __global__ void blake512_gpu_hash(int threads, uint32_t startNounce, void *outputHash, uint32_t *heftyHashes, uint32_t *nonceVector)
{
	...
	if (thread < threads)
	{
		...
		// 128 Byte fr die Message
		uint64_t buf[16];

		// Message fr die erste Runde in Register holen
#pragma unroll 16
		...
		if (BLOCKSIZE == 84) {
			...
			// aufwã­¤ig, weil das nicht mit uint64_t Wí¶‡í³¥rn aligned ist.
...
__host__ void blake512_cpu_init(int thr_id, int threads)
{
	...
	// Speicher fr alle Ergebnisse belegen
	...
	// heftyHash hat 32-Byte
{
	...
	if (len == 84) {
		// Message mit Padding fr erste Runde bereitstellen
		memcpy(PaddedMessage, pdata, 84);
		memset(PaddedMessage+84, 0, 32); // leeres Hefty Hash einfllen
		memset(PaddedMessage+116, 0, 12);
		PaddedMessage[116] = 0x80;
	} else if (len == 80) {
		memcpy(PaddedMessage, pdata, 80);
		memset(PaddedMessage+80, 0, 32); // leeres Hefty Hash einfllen
...
__host__ void blake512_cpu_hash(int thr_id, int threads, uint32_t startNounce)
{
	...
	// Grí¼ºí³ des dynamischen Shared Memory Bereichs
